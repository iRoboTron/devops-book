...
## 5.4 StorageClass и динамическое выделение

`StorageClass` — это **рецепт, как автоматически создавать диски** под PVC. Без него администратору пришлось бы заранее руками создавать каждый `PersistentVolume` (это называется *статическое выделение*). С ним всё проще: ты просто просишь «дай 1Gi», а **provisioner** сам создаёт том под капотом — это *динамическое выделение (dynamic provisioning)*.
...
Слово `(default)` означает: если в PVC не указать `storageClassName`, возьмётся этот класс. Поэтому в k3s PVC обычно «просто работает» — `local-path` стоит дефолтным.

**`reclaimPolicy` — что станет с диском, когда удалишь PVC:**

| Политика | Что происходит | Когда нужна |
|---|---|---|
| `Delete` | Диск и **данные удаляются** вместе с PVC | временные данные, dev |
| `Retain` | PVC удалён, а **диск с данными остаётся** | боевая БД, важные данные |

> **Запомни:** дефолтный `local-path` в k3s — это `Delete`. Удалил PVC с базой → данные исчезли. Для важного держи отдельный том с `reclaimPolicy: Retain` (или, что надёжнее, регулярный бэкап — см. Том 5).

Есть ещё параметр `volumeMode`: по умолчанию `Filesystem` (том монтируется как папка) — это то, что нужно в 99% случаев. `Block` (сырое блочное устройство) нужен редко, специальным БД.
