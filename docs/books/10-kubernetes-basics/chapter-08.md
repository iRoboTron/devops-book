...
## 10.4 QoS-классы — кого убьют первым

По тому, как ты задал `requests` и `limits`, Kubernetes сам присваивает Pod'у **класс качества обслуживания (QoS)**. Это решает, кого «выселят» с ноды первым, когда памяти начнёт не хватать:

| Класс | Условие | При нехватке памяти |
|---|---|---|
| **Guaranteed** | `requests` == `limits` у всех контейнеров | убивают **последним** |
| **Burstable** | `requests` заданы, но меньше `limits` | посередине |
| **BestEffort** | вообще нет `requests`/`limits` | убивают **первым** |
...
> **Запомни:** хочешь, чтобы важный сервис (например, база) выживал при нехватке памяти — задай ему `requests` == `limits`, тогда он попадёт в **Guaranteed**. Pod'ы без ресурсов (**BestEffort**) — первые кандидаты на вылет.
...
- [ ] Я понимаю QoS-классы (Guaranteed / Burstable / BestEffort)

**Всё отметил?** Переходи к Главе 11 — Диагностика: когда что-то сломалось.
