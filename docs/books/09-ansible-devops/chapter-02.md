...
Логику любого идемпотентного модуля можно изобразить так: сначала проверяется текущее состояние, и только при расхождении вносится изменение.

```mermaid
flowchart TD
    A["задача\n(модуль + желаемое\nсостояние)"] --> B{"состояние\nуже верное?"}
    B -->|"да"| C["ok\nchanged: false"]
    B -->|"нет"| D["внести изменение"]
    D --> E["changed: true"]
    A -.->|"хост не отвечает\nпо SSH"| F["unreachable"]
...
Именно поэтому повторный запуск даёт `changed: false`: состояние уже совпадает с желаемым, и модуль ничего не трогает.
