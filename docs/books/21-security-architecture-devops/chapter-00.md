...
Архитектурное мышление идёт от актива к контролю: сначала определяем, что защищаем, затем границу доверия вокруг него, потом цель контроля и конкретный механизм.

```mermaid
flowchart LR
    A["Актив\n(данные, сервис, доступ)"] --> B["Trust boundary\n(где меняется доверие)"]
