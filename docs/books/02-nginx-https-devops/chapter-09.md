...
Тип проекта: **отдельный проект с чистой Ubuntu**.

Ты не продолжаешь итоговый проект книги 1. Даже если в книге 1 уже было Python-приложение, здесь мы специально начинаем заново: новый пользователь приложения, новый каталог `/var/www/myapp`, новый `requirements.txt`, новый systemd-сервис и новый Nginx-конфиг.

До начала должно быть:
- чистая Ubuntu Server 22.04 или 24.04;
- пользователь с `sudo`;
- SSH-доступ к серверу;
- домен для реального HTTPS или локальное имя `myapp.local` для тренировки;
- открыто второе SSH-окно перед включением `ufw`.

Если ты используешь VM после книги 1, лучше откатись к снапшоту «Clean Install» или создай новую VM. Так ты не будешь чинить конфликты старого systemd-сервиса, старого Nginx-конфига и старых Python-зависимостей.
...
> **Хорошая точка для снапшота.** К этому моменту у тебя рабочий Python-сервис, Nginx с HTTPS, ufw и кастомная страница ошибки. Если используешь VirtualBox или другой гипервизор — сохрани состояние VM прямо сейчас: `Машина → Сделать снимок состояния`. Это позволит вернуться к рабочей конфигурации, если что-то пойдёт не так в следующих шагах.
