...
Тип проекта: **учебная практика на текущей Linux-VM**.

Это не база для следующей книги. В книге 2 итоговый проект начинается заново с чистой Ubuntu и проходит Nginx/HTTPS уже как отдельный сетевой проект.

До начала должно быть:
- Ubuntu Server или учебная VM из этой книги;
- пользователь с `sudo`;
- доступ к терминалу;
- базовое понимание `systemctl`, прав файлов и `nano`.

Если VM уже сильно менялась, лучше откатись к снапшоту перед этой главой или создай новую учебную VM. Здесь не требуется домен, HTTPS, Docker или GitHub Actions.
...
> Виртуальное окружение и зависимости ставь от имени этого пользователя, иначе потом легко столкнуться с проблемами прав доступа.
...
> **Запомни:** Gunicorn — сервер для продакшна Python-приложений.
