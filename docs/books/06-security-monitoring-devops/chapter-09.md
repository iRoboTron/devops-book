...
## 9.5 Финальное сохранение VM

После финальной проверки сделай последний снапшот VM. Это уже не обязательная входная точка для следующей книги, а сохранённое состояние полностью пройденного курса: Docker, CI/CD, бэкапы, безопасность, мониторинг и runbook.
...
```text
after-book-6-hardened-myapp
```

Этот снапшот полезен как точка возврата перед самостоятельными экспериментами: обновлением Docker Compose, сменой домена, переносом проекта, настройкой новых алертов или тестами аварийного восстановления.

---

## 9.6 Поздравляю! Курс завершён
