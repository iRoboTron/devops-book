...
## 9.10 Сохранение VM для книги 5

После успешного CI/CD-пайплайна сделай снапшот VM. Это входная точка для книги 5: там ты будешь доводить этот же сервер до production-ready состояния, добавлять бэкапы, миграции, лимиты, health-check и обслуживание.
...
- пользователь `deploy`;
- рабочий SSH-доступ для GitHub Actions;
- настроенные workflow для тестов и деплоя;
- rollback-скрипт;
- `.env` на сервере, но не в git.

Если в книге 5 ты ошибёшься с бэкапами, миграциями или настройками PostgreSQL, откат к этому снапшоту вернёт сервер к рабочему состоянию после CI/CD.
