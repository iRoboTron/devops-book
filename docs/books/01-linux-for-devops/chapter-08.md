...
## 8.9 Как читать ошибку: алгоритм

В разделе 8.7 мы разобрали конкретные ошибки. Но в реальной работе ты встретишь сотни разных сообщений, которых нет ни в одном списке. Поэтому важнее не заучить ошибки, а **выработать привычку их читать**. Это и есть главный навык DevOps — не «знать ответ», а уметь его найти.

> **Запомни:** Ошибка — это не «всё сломалось», а подсказка, где именно сломалось. Сначала прочитай её буквально, потом сужай круг.

Алгоритм из 4 шагов:

1. **Прочитай ошибку буквально, до последней строки.** У Python и многих программ настоящая причина — в самом низу (`...`, потом `Error: ...`). Не пугайся «простыни» — тебе нужна последняя осмысленная строка.
2. **Определи слой, на котором сломалось.** Запрос проходит через несколько слоёв: DNS → сеть/firewall → TLS/сертификат → сам сервис → приложение → данные. Ошибка почти всегда указывает на один из них.
3. **Сужай по одному шагу.** Проверяй слои по очереди, от внешнего к внутреннему, и смотри, на каком обрывается.
4. **Меняй одно за раз.** Поправил одно — проверил. Не меняй пять вещей сразу: потом не поймёшь, что помогло.

**Пример: `curl https://myapp.ru` не работает.** Идём по слоям:

```text
curl https://myapp.ru
│
├─ "Could not resolve host"     → слой DNS.   Проверь: nslookup myapp.ru, /etc/resolv.conf
├─ "Connection refused/timeout" → слой сети.  Проверь: ss -tlnp | grep 443, ufw status, жив ли сервис
├─ "SSL certificate problem"    → слой TLS.   Проверь: срок сертификата, цепочку, дату на сервере
├─ "502 Bad Gateway"            → слой прокси. Проверь: жив ли backend за Nginx, его логи
└─ "500 Internal Server Error"  → слой приложения. Проверь: логи приложения (journalctl -u myapp)
```

Один и тот же `curl` даёт **разные** ошибки в зависимости от того, какой слой сломан. Научишься их различать — и половина проблем перестанет быть «магией».

> **Запомни:** «Не работает» — это не диагноз. Диагноз — это «не резолвится DNS» или «сервис не слушает порт». Доводи до конкретного слоя.

Этот же образ мышления масштабируется на всю эксплуатацию: в **Томе 28 «Культура DevOps/SRE»** он превращается в runbook'и (готовые пошаговые проверки) и blameless-постмортемы (разбор «почему сломалось» без поиска виноватых).
