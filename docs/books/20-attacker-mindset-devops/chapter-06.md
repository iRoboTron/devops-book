...
В плоской сети одна компрометация открывает почти всё. Сегментация и раздельные доступы ограничивают blast radius: атакующему приходится преодолевать новую границу на каждом шаге.

```mermaid
flowchart LR
    subgraph flat["Flat network (риск)"]
        h1["Скомпрометирован-\nный хост"]
        h2["App server"]
        h3["DB"]
        h4["Backups"]
    ...
    subgraph segmented["Сегментация (защита)"]
        s1["Скомпрометирован-\nный хост"]
        gw["Граница зоны /\njump host"]
        s2["DB / storage"]
        s1 --> gw
    ...
    style s1 fill:#6e2f1a,color:#fff
    style gw fill:#1e8449,color:#fff
```

Раздельные ключи per host и отдельный management plane не дают одной утёкшей учётке открыть всю инфраструктуру.
