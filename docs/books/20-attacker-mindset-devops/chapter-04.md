...
- слишком широкие sudoers rules;
- неправильные права на каталоги и скрипты служб;
- неожиданные setuid и setgid бинарники;
- службы запускаются от root без причины;
- секреты root-процессов доступны обычному пользователю.
...
```bash
printf 'document top local privilege risks
'
```
