...
Docker проверяет слои сверху вниз: как только один слой изменился — все слои ниже пересобираются, даже если их содержимое не менялось.

```mermaid
flowchart TD
    A["Слой 1: COPY requirements.txt ."] --> checkA{Файл изменился?}
    checkA -->|Нет| cacheA["✓ Берём из кэша"]
    checkA -->|Да| rebuildA["⟳ Пересобрать"]

    cacheA --> B["Слой 2: RUN pip install"]
    rebuildA --> rebuildB["⟳ Пересобрать\n(зависит от слоя выше)"]

    B --> checkB{Команда изменилась?}
    checkB -->|Нет| cacheB["✓ Берём из кэша"]
    checkB -->|Да| rebuildB2["⟳ Пересобрать"]

    cacheB --> C["Слой 3: COPY . ."]
    rebuildB --> C
    rebuildB2 --> C

    C --> checkC{Код изменился?}
    checkC -->|Нет| cacheC["✓ Берём из кэша"]
    checkC -->|Да| rebuildC["⟳ Пересобрать"]
```

Именно поэтому `COPY . .` ставят последним: изменение кода ломает кэш только одного слоя, а не тянет за собой долгий `pip install`.
